...
def delta_avoid(current,avoid):
    theta = np.arctan2((avoid[1]-current[1]),(avoid[0]-current[0]))
...
def delta(current):
    ...
    del_a = delta_avoid(current,avoid)
    return (del_a[0]+del_g[0],del_a[1]+del_g[1])

@np.vectorize
def delta_copy(x,y):
    return delta((x,y))
xq,yq= np.meshgrid(np.arange(-1,11,0.3),np.arange(-1,11,0.3))
